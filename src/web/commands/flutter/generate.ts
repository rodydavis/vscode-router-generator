...
import { readFile, getSetting } from "../utils";
import { addHeader } from "../utils/comments";
import { StringBuilder } from "../utils/string-builder";
import { pagesToWidgets } from "./analyze";

export async function generateFlutter(root: string, pages: PageRoute[]) {
    const sb = new StringBuilder();
    const add = (line: string) => sb.writeln(line);
    const empty = () => sb.writeln();
    const components = pagesToWidgets(pages);
    addHeader(sb, components);
    let title = getSetting('title');
    // Read title from yaml
    if (!title) {
        const yaml = await readFile(`${root}/pubspec.yaml`);
        const yamlObj = yaml.split('\n').reduce((acc, line) => {
            const [key, value] = line.split(':');
            if (key) {
                Object(acc)[key.trim()] = value.trim();
            }
            return acc;
        }, {});
        title = Object(yamlObj)['name'];
    }
    title ??= '';
    ...
    for (const p of components) {
        p.alias = `route${i}`;
        add(`import "${p.relativePath}" as ${p.alias};`);
        i++;
    }
    empty();
    ...
    for (let i = 0; i < components.length; i++) {
        ...
        add(`    pages['${entry.route}'] = route${i}.${name}();`);
    }
    add("    SystemNavigator.selectSingleEntryHistory();");
    add("    loadRoute();");
    sb.writeAll([
        'if (kIsWeb) {',
        '    html.loadLibrary().then((value) {',
    ...
    sb.writeAll([
        ...
        ``,
        `for (final page in pages.entries.toList()) {`,
